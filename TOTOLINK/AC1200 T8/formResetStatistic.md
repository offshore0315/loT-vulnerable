...
In the D-Link DIR-605L 2.13B01 BETA firmware has a buffer overflow vulnerability in the `formResetStatistic` function. The `Var` variable receives the `curTime` parameter from a POST request. However, since the user can control the input of `curTime`, the `sprintf` can cause a buffer overflow vulnerability.
